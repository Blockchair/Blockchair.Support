...
Новая версия не является обратно совместимой с предыдущей для многих случаев, пожалуйста, ознакомьтесь с документацией.

* До 1 сентября 2018, `api.blockchair.com` будет отдавать API v.1
* До 1 сентября 2018, `alpha-2.blockchair.com` будет отдавать API v.2 в бета-режиме
* Начиная с 1 сентября 2018, `api.blockchair.com` будет отдавать API v.2 в продакше-режиме
* Начиная с 1 сентября 2018, `api-v1.blockchair.com` будет отдавать API v.1 (это будет поддерживаться как минимум месяц)

Пожалуйста, протестируйте свои приложения и сделайте необходимые изменения. В случае вопросов, свяжитесь с нами.

##### Глобальные изменения

* Выдаваемый результат по всем запросам унифицирован (ответ от сервера теперь всегда возвращает JSON-массив состоящий из двух подмассивов: `data` с непосредственно результатом, и `context` с мета-информацией).
* Типы выдаваемых значений приведены к корректным (например, числа всегда возвращаются в integer, а не в string)
* Для всех запросов скорость работы увеличена от 3 до 10 раз
* Множество новых удобных dashboard-коллов!

##### Нововведения API работы с блокчейнами (`bitcoin[-cash]/[mempool/](blocks|transactions|outputs)`, `ethereum/[mempool/](blocks|uncles|transactions|calls)`)

* Доступно до 2 одновременных сортировок, которые можно перечислять через запятые (пример: `?s=field1(asc|desc),field2(asc|desc)`)
* Доступно до 5 одновременных фильтров
* Секция `?limit=N` позволяет теперь указывать `N` от 1 до 100. По умолчанию - 10. Примечание: секция `LIMIT` игнорируется в некоторых случаях, тогда в `context.limit` будет `null`
* Введён оператор `...` для сторогого сравнения, например `?q=id(1...3)` вернёт только 2, `?q=id(1...)` вернёт всё начиная с 2
* Введены операторы `OR` и `NOT`
* Отменены секции `?next=N` и `?next_sort=M`, вместо них теперь используется секция `OFFSET`, например: `?offset=10` для получения следующих 10 записей после стандартного лимита в 10. Максимальное значение `OFFSET` равно 100000.
* Контекстная информация перенесена в специальный массив `context`, который возвращает API. Часть информации поменяло ключи.
